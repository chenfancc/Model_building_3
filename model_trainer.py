...
class TrainModel:
    ...
    def train_one_epoch(self):
        ...
        self.model.train()
        for inputs, baseline_inputs, labels in self.train_dataloader:
            inputs, baseline_inputs, labels = inputs.to(DEVICE), baseline_inputs.to(DEVICE), labels.to(DEVICE)

            self.optimizer.zero_grad()
            outputs = self.model(inputs.float(), baseline_inputs.float())
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for inputs, baseline_inputs, targets in self.valid_dataloader:
                count += 1
                inputs, baseline_inputs, targets = inputs.to(DEVICE), baseline_inputs.to(DEVICE), targets.to(DEVICE)
                outputs = self.model(inputs.float(), baseline_inputs.float())
