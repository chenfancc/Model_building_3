...
class BalancedData:
    def __init__(self, data, baseline, labels):
        self.data = data
    ...
    def oversample(self):
        ...
        baseline_balanced = self.baseline[indices]
        labels_balanced = self.labels[indices]

        return data_balanced, baseline_balanced, labels_balanced
    ...
    def smote(self):
        ...
        baseline_numpy = self.baseline.numpy()
        labels_numpy = self.labels.numpy()

        # 获取数据的形状
        original_shape_data = data_numpy.shape
        original_shape_baseline = baseline_numpy.shape

        # 将数据重塑为二维
        data_reshaped = data_numpy.reshape(original_shape_data[0], -1)
        baseline_reshaped = baseline_numpy.reshape(original_shape_baseline[0], -1)

        smote = SMOTE(random_state=42)
        data_resampled, baseline_resampled, labels_resampled = smote.fit_resample(data_reshaped, baseline_reshaped, labels_numpy)

        # 将数据重塑回原来的形状
        data_resampled = data_resampled.reshape(-1, *original_shape_data[1:])
        baseline_resampled = baseline_resampled.reshape(-1, *original_shape_data[1:])

        data_balanced = torch.from_numpy(data_resampled)
        baseline_balanced = torch.from_numpy(baseline_resampled)
        labels_balanced = torch.from_numpy(labels_resampled)
        return data_balanced, baseline_balanced, labels_balanced

    def origin(self):
        data_balanced = self.data
        baseline_balanced = self.baseline
