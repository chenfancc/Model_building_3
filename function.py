...
def main_data_loader(data_dir, sample_method):
    ...
    baseline_train = data['baseline_tensor_train']
    label_train = data['label_tensor_train']

    data_val = data['data_tensor_val']
    baseline_val = data['baseline_tensor_val']
    label_val = data['label_tensor_val']

    data_test = data['data_tensor_test']
    baseline_test = data['baseline_tensor_test']
    label_test = data['label_tensor_test']

    balancer = BalancedData(data_train, baseline_train, label_train)
    data_train_b, baseline_train_b, label_train_b = balancer.sample(method=sample_method)

    dataset_train = TensorDataset(data_train_b, baseline_train_b, label_train_b)
    dataset_val = TensorDataset(data_val, baseline_val, label_val)
    dataset_test = TensorDataset(data_test, baseline_test, label_test)
